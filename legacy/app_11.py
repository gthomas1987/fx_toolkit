"""App 11 — Leveraged FX Exotic Options Portfolio Risk Monitor.

Monitors a portfolio of European-style FX options: vanillas, call spreads,
call flys, EKOs, and dual EKOs. Risk dashboards aligned to what actually
matters for an exotic book: bucketed Greeks, barrier/path risk, correlation
risk, and scenario cubes.

Run:
    cd /home/claude/fx_levels_monitor
    streamlit run apps/app_11.py
"""
from __future__ import annotations
import sys
from pathlib import Path

# Make `core` importable when streamlit launches us from the apps/ dir.
...
DATA_DIR = PROJ_ROOT / "market_data"
...
st.set_page_config(
    page_title="App 11 — Exotic FX Options Risk Monitor",
    layout="wide",
...
def _available_dates_cached() -> list[pd.Timestamp]:
    return available_dates(str(DATA_DIR))


@st.cache_data(show_spinner=False)
def _snapshot_cached(asof_iso: str) -> dict:
    return snapshot(str(DATA_DIR), pd.Timestamp(asof_iso))


@st.cache_data(show_spinner=False)
def _price_all_cached(asof_iso: str) -> pd.DataFrame:
    """Price the full portfolio and return a tidy DataFrame."""
    asof = pd.Timestamp(asof_iso)
    snap = _snapshot_cached(asof_iso)
...
@st.cache_data(show_spinner=False)
def _greeks_all_cached(asof_iso: str) -> pd.DataFrame:
...
@st.cache_data(show_spinner=False)
def _barriers_all_cached(asof_iso: str) -> pd.DataFrame:
    asof = pd.Timestamp(asof_iso)
    snap = _snapshot_cached(asof_iso)
    trades = build_portfolio()
    rows = []
...
all_dates = _available_dates_cached()
...
st.title("🎯 Leveraged FX Exotic Options — Risk Monitor (App 11)")
st.markdown(
...
with TAB_OVERVIEW:
    ...
    bdf = _barriers_all_cached(asof_iso)
...
with TAB_GREEKS:
    ...
    gdf = _greeks_all_cached(asof_iso)
...
with TAB_BARRIER:
    ...
    if bdf.empty:
        ...
        pdf = _price_all_cached(asof_iso)[["trade_id", "notional_usd"]]
        ...
        snap_now = _snapshot_cached(asof_iso)
...
with TAB_CUBE:
    ...
    pdf = _price_all_cached(asof_iso)
...
with TAB_DRILL:
    ...
    if trade.structure == "dual_eko":
        ...
        def _dual_mtm_surface(trade_id: str, asof_iso: str,
            ...
            levels."""
            asof_ts = pd.Timestamp(asof_iso)
            snap_local = _snapshot_cached(asof_iso)
    ...
    hist_dates = [d for d in _available_dates_cached()
    ...
    if len(hist_dates) > 1:
        ...
        for d in hist_dates:
            try:
                sp = _snapshot_cached(d.strftime("%Y-%m-%d"))
