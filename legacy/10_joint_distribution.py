"""Joint Distribution Analysis — Streamlit app.

Standalone implementation of the framework in
`joint_distribution_tutorial.docx`, applied to USDJPY × USDKRW (or any
two-pair combination found in the market_data folder).
...
Run:  streamlit run apps/10_joint_distribution.py
Env:  MARKET_DATA_DIR=<path-to-market_data>
"""
...
# Project root for core imports (this file is apps/10_..., so parent.parent is repo)
HERE = Path(__file__).resolve().parent
ROOT = HERE.parent
sys.path.insert(0, str(ROOT))

from core.data_loader import load_panel, discovery_summary   # noqa: E402
...
# Page / theme
# =============================================================================
st.set_page_config(layout="wide", page_title="Joint Distribution Analysis")

# Subtle dark-mode-friendly chart palette (consistent with app 9)
CHART_BG = "#0e1117"
...
# Header
# =============================================================================
st.title("🪐 Joint distribution analysis — two-pair FX")
st.caption(
    "Standalone tool for understanding the joint historical distribution of "
    "two FX pairs (default: USDJPY × USDKRW). Implements the framework from "
    "`joint_distribution_tutorial.docx`: 2D KDE → GMM clusters → HMM regime "
    "dynamics → Mahalanobis-based barrier guidance. Each tab includes a "
...
folder = st.sidebar.text_input(
    "Market data folder",
    value=default_folder or str(ROOT / "market_data"),
    help="Same `market_data` folder used by the other apps "
          "(must contain `_index.csv`).",
    key="jd_folder",
)
if not folder or not Path(folder).exists():
    st.sidebar.error(f"Folder does not exist: {folder!r}")
    st.error("Specify a valid market data folder in the sidebar.")
    st.stop()
