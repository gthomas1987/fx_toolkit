"""App 1d — FX Forward-Point Butterflies (curvature).

Run with:
    streamlit run apps/1d_fwd_butterflies.py

Butterflies (2 × body − wings):
    1W-1M-3M  ·  1M-3M-6M  ·  3M-6M-1Y

Positive butterfly = body fwd-point is rich relative to its wings (curve
is convex/humped). Negative = body is cheap (curve is concave/V-shape).

Tabs:
    1. Time series — selected butterflies with quantile bands & pct path
    2. Snapshot — current values + percentiles for all butterflies
    3. Distribution — per-butterfly histogram with current marker
"""
...
from core.fwd_structures import all_butterflies, BUTTERFLY_SPECS
...
# -----------------------------------------------------------------------------
st.set_page_config(
    page_title="1d — FX Forward Butterflies",
    layout="wide",
    initial_sidebar_state="expanded",
)
app_header(
    "1d · FX Forward-Point Butterflies",
    "Butterflies:  1W-1M-3M  ·  1M-3M-6M  ·  3M-6M-1Y    "
    "(2 × body − short − long; positive = rich body)",
)
...
# Load & build
# -----------------------------------------------------------------------------
@st.cache_data(show_spinner="Loading forward points and building butterflies…")
def load_and_build(folder: str, pair: str, prefer: str
    ...
    for tenor in FWD_TENORS:
        ...
        if not df.empty and pair in df.columns:
            ...
            if not s.empty:
                fwd_panels[tenor] = s
    return fwd_panels, all_butterflies(fwd_panels)


fwd_panels, butterflies = load_and_build(folder, pair, prefer)
if not butterflies:
    st.warning(
        f"No butterflies could be built for {pair}. "
        f"Available fwd tenors: {sorted(fwd_panels.keys())}"
    )
    st.stop()

bf_labels = [lbl for (lbl, *_) in BUTTERFLY_SPECS if lbl in butterflies]
...
with tab_ts:
    ...
    if not sel:
        st.info("Select at least one butterfly.")
        st.stop()

    cutoff = max((butterflies[l].index[-1] for l in sel)) - pd.Timedelta(days=lookback_days)
    wide = pd.concat({l: butterflies[l] for l in sel}, axis=1)
    wide_cut = wide[wide.index >= cutoff]

    fig = time_series_chart(
        wide_cut,
        title=f"{pair}  ·  Forward-point butterflies  ·  trailing {lookback_label}",
        yaxis_title="Butterfly (pip units)",
        height=460,
    )
    st.plotly_chart(fig, use_container_width=True)

    st.markdown("##### Per-butterfly: value + reference quantiles + percentile path")
    for label in sel:
        s = butterflies[label]
...
with tab_snap:
    ...
    styled = (df.style
                .format({c: "{:,.2f}" for c in df.columns if c != "Butterfly"})
...
# ============================================================================
with tab_dist:
    sel_label = st.selectbox("Butterfly", bf_labels, key="fb_dist_sel")
    if sel_label and sel_label in butterflies:
        s = butterflies[sel_label]
        cutoff = s.index[-1] - pd.Timedelta(days=lookback_days)
...
st.caption(
    ...
    + "  ·  Construction: 2 × body − short − long"
)
